...
#*This is the function to open the second window once the name is verified
def open_window_1():
    window.withdraw()
    ...
    window_1 = Toplevel(window)#*This will make it so that the window_1 window, is a sub window of the original window
    window_1.title("Place Survey")#*The windows tile
    window_1.state("zoomed")#*Makes sure the window is full screen
    window_1.configure(bg="#E3F6F5")#*This sets the background color of the quiz

    inner_frame = tk.Frame(window_1, width = 1000, height = 550, bg = "#FFFFFE", highlightbackground = "#272343", highlightthickness = 2)#*This is the code to create the inner frame for aesthetic and organizational purposes
    inner_frame.place(relx = 0.5, rely = 0.57, anchor = CENTER)#*This centralizes the frame

    #*This is the code for the header
    label_1 = tk.Label(window_1, text="PLACE SURVEY", font=("Arial", 60, "bold"))
    label_1.pack(padx=20, pady=20)
    label_1.configure(bg="#E3F6F5", fg="#272343")

    #*This is the text to ask the first question
    text_1 = tk.Label(window_1, text="What Part Of Auckland Do You Live In", font=("Arial", 30))
    ...
    dp_1 = ttk.Combobox(window_1, values=city_prt, state="readonly", font=("Arial", 20))
    ...
    text_2 = tk.Label(window_1, text="Which Suburb In Auckland Do You Live In Or Near", font=("Arial", 30))
    ...
    dp_2 = ttk.Combobox(window_1, values=["Select Suburb"], state="readonly", font=("Arial", 20))
    dp_2.current(0)
    dp_2.pack(pady=40)
    ...
    btn = tk.Button(window_1, text="Get Locations To Study", font=("Arial", 35), command= show_place)
    btn.pack(padx=20, pady=20)

#*This is the function to validate the name in the first window
def val_name():
    name = name_entry.get()
    if 4 <= len(name) <= 20 and name.isalpha():#*Checks if the name has 4-20 letters and that it only contains alphabets
        open_window_1()
        messagebox.showinfo("Successful", "Welcome to the Place Survey")
    else:
        messagebox.showerror("Error", "The name must be between 4 - 20 letters long" + "\n" + "The name can only contain alphabets"+"\n"+"Please Try Again")
...
#*This is the label for the first window to ask the question
label = tk.Label(window, text="Please Enter Your Name", font = ("Arial", 60, "bold"))
label.pack(padx = 20, pady = 20)
label.configure(bg = "#E3F6F5", fg = "#272343")

#*This is the code to create a entry field for the user to input into
name_entry = tk.Entry(window, font = ("Arial", 50))
name_entry.pack(pady = 70)

#*This is the button that will carry out the val_name function
val_button = tk.Button(window, text="CLICK TO CONTINUE", font = ("Arial", 40), command=val_name)
val_button.pack(pady = 90)

#*This line of code makes sure that the window is displayed when the code is run
window.mainloop()